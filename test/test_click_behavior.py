...
from test_utils import get_cr_window_region, manual_input_window_region
...
def record_click_positions():
    ...
    # 尝试自动获取皇室战争窗口区域
    cr_window = get_cr_window_region(click_manager)
    
    if not cr_window:
        print("\n请手动输入皇室战争窗口区域信息：")
        cr_window = manual_input_window_region()
        if not cr_window:
            return
