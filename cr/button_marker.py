...
class ButtonMarker:
    ...
    def __init__(self):
        self.config = BUTTON_MARK_CONFIG
        self.ocr_config = DOUBAO_OCR_CONFIG
    ...
    def verify_with_doubao(self, image_path):
        """使用豆包验证按钮位置是否正确"""
        # 使用用户提供的OCR工具调用豆包
        ocr_cmd = f"{self.ocr_config['python_path']} {self.ocr_config['ocr_script_path']} {image_path} --question '图片中红色边框标记的位置是否是按钮？请回答是或否。'"
